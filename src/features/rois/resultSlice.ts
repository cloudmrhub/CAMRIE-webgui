    ...
    extraReducers: (builder) => (
        ...
        builder.addCase(
            loadResult.fulfilled, (state: ROIState, action) => {
                // Clone the job object to avoid mutating a frozen state
                const clonedJob = { ...action.payload.job };

                // Update properties on the cloned object
                clonedJob.setup = { alias: '-', version: 'v0' };
                clonedJob.setup.task = action.payload.result.headers.options;
